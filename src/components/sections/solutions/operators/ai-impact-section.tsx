...
interface OperatorsAiImpactSectionProps {
  data?: {
    title?: string;
    subtitle?: string;
    impactTitle?: string;
...
export function OperatorsAiImpactSection({ data }: OperatorsAiImpactSectionProps) {
        ...
        <div className="relative z-10 w-full px-4 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {data.metrics?.filter(metric => metric.value && metric.label).map((metric, index) => {
              const icons = [TrendingUp, Zap, Shield, Users];
              const variants = ['accent', 'primary', 'secondary', 'success'] as const;
